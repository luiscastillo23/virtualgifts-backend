...
export class CategoriesService {
  ...
  async create(
    ...
    try {
      ...
      const existingCategoryByName: Category =
      ...
      const existingCategoryBySlug: Category =
        await this.prisma.category.findFirst({
      ...
      const imageKey: string = `categories/images/${Date.now()}-${sanitizedImageFilename}`;

      // Upload the image to S3
      const uploadResult: any = await this.s3Service.uploadFile(
        image,
        imageKey,
      );
  ...
  async findAll(): Promise<Category[]> {
    try {
      ...
      this.logger.error('Failed to fetch all roles', (error as Error)?.stack);
  ...
  async findOne(id: string): Promise<Category> {
    try {
      ...
      this.logger.error('Failed to fetch category', (error as Error)?.stack);
  ...
  async update(
    ...
    try {
      ...
      if (image) {
        ...
        imageKey = `categories/images/${Date.now()}-${sanitizedImageFilename}`;
  ...
  async remove(id: string): Promise<{ success: boolean }> {
    ...
    try {
      ...
      const subcategoriesCount: number = await this.prisma.subcategory.count({
        where: { categoryId: id },
      });
  ...
  async findProducts(id: string) {
    ...
    try {
      ...
      this.logger.error('Failed to fetch products', (error as Error)?.stack);
